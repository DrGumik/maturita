...
void serialWrite(int character, int dispType)
{
	if(dispType)
	{
		...
		{
			writeData(character & 0x01);      // Maskovany LSB bit z digit a nastaveny jako DATA
	...
	else
	{
		for (int i = 0; i < 7; i++)          // Smycka pro serializaci dolnich 7 bitu promenne digit
		{
			if ((character & 0x01) != 0) 
				...
				writeData(0);	// Maskovany LSB bit z digit a nastaveny jako DATA
