...
namespace tic_tac_toe_tenk
{
    public partial class GameBoard : UserControl
    {
        ...
        private GameType gameType = GameType.PvP; // Typ hry - PvP / PvA
        ...
        {
            // Vygeneruje a připraví nové pole GameField pro hru
            if (boardSize < 21)
        ...
        {
            // Zavolani funkce k vygenerovani herni plochy
        ...
        private bool CheckWinner(int numberOfFieldRows)
        {
            ...
            {
                MessageBox.Show("Vyhrál hráč s kamenem: " + currentPlayer);
        ...
        {
            // Kontrola, pokud není tah výherní, tak zda je herní pole plné
            int isGameBoardFull = 0;

            for (int x = 0; x < boardSize; x++)
            {
                for (int y = 0; y < boardSizeY; y++)
                {
                    if (gameFieldsBoard[x, y] == GameField.NotOccupied)
                        isGameBoardFull++;
                }
            }

            if (isGameBoardFull == 0)
            {
            ...
            if (posY == 0 && posX <= boardSize) 
            {
                for (int y = boardSizeY; y >= 0; y--)
                {
                    if (gameFieldsBoard[posX, y] == GameField.NotOccupied)
                    {
                        ...
                        // Pokud je gameType PvA, tak po vykonani tahu hrace hraje AI
                        if (gameType == GameType.PvA)
                        {
                            if (gameState != GameState.Played)
                            {
                                currentPlayer = currentPlayer == GameField.X ? GameField.O : GameField.X;
        ...
        {
            // Pokus o AI algoritmus pro hrani PvA
            int AIposY = 0;
            int AIposX = 0;

            // Najde nejlepsi misto
            FindBestMove(posX, posY, ref AIposX, ref AIposY);

            // Vlozi kamen na vyhodnocene misto, zkontroluje zda AI vyhralo nebo ne a pokracuje hrac
        ...
        private void FindBestMove(int posX, int posY, ref int AIposX, ref int AIposY) 
        {
            ...
            while (tryAgain) {
                ...
                for (AIposX = 0; AIposX < boardSize; AIposX++)
                {
                    for (int y = boardSizeY; y >= 0; y--)
                    {
                        // Kontrola zda je misto volne a dale zda s nejakym kamenem sousedi
