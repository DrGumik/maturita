...
namespace tic_tac_toe_tenk
{
    public partial class GameBoard : UserControl
    {
        ...
        {
            // AI algoritmus, který je fakt špatný
            int AIposY = 0;
            int AIposX = 0;
        ...
        private void FindBestMove(int posX, int posY, ref int AIposX, ref int AIposY) 
        {
            ...
            for (int y = boardSizeY; y >= 0; y--)
            {
                if (gameFieldsBoard[x, y] == GameField.NotOccupied)
                {
                    if (x - 1 >= 0) 
                    { 
                        if (gameFieldsBoard[x-1, y] == GameField.X)
                        {
                            score = 10;
                            
                            if (x - 2 >= 0) 
                            { 
                                if (gameFieldsBoard[x - 2, y] == GameField.X)
                                {
                                    score = 20;
                                    AIposY = y;
                                    break;
                        ...
                        {
                            if (gameFieldsBoard[x - 1, y] == GameField.O)
                            {
                                score = 10;

                                if (x - 2 >= 0)
                                {
                                    if (gameFieldsBoard[x - 2, y] == GameField.O)
                                    {
                                        score = 20;
                                        AIposY = y;
                                        break;
