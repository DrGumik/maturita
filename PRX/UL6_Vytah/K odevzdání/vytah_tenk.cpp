...
int main()
{
  ...
  while (pressedKey != 'q')
  {
    ...
    if (!checkDoorState())
    {
      ...
      if (moveState) // Pokud kabina je v pohybu
      {
        if (position == btnState) // Pokud kabina dorazi do patra
        {
          moveState = 0;
          buzzerState = 0;
          buzzerCounter = 500;
      ...
      {
        if (position < btnState) // Posun nahoru
        {
          moveState = 1;
          moveData = 0x0A;
          printf("\n\r Posun nahoru do patra c. %i", btnState);
        }
        else if (position > btnState) // Posun dolu
        {
          moveState = 1;
          moveData = 0x01;
