...
int controlDisplayBtns(int clickedBtnPosition, int& segmentDelayCounter, int& position, int& lastPushedBtn, int& testBtnDelay) {
	clickedBtnPosition = 0;

	if (position < 4) {
		if (segmentDelayCounter > 0) {
			segmentDelayCounter--;
		}		
		else {
			position++;
			...
			int tmp = inportb(P3IN);	// Precteni portu, kde jsou pripojena tlacitka

			// Maskovani prectene hodnoty a ziskani informace zda je tlacitko stisknuto
			if ((tmp &= door_btn_masks[1]) == 1) 
			{
				clickedBtnPosition = position;

				// Podminka pro detekci nabezne hrany tlacitka + delay mezi kontrolami tlacitek (40 * delta T = +-80ms)
...
int main()
{
	...
	while(pressedKey != 'q' && pressedKey != 'Q') {	
		...
		int clickedBtnPosition = controlDisplayBtns(clickedBtnPosition, segmentDelayCounter, position, lastPushedBtn, testBtnDelay);
		...
		if (doorState == 0) {
			...
			switch(settingsState) {
				case 0: // Základní stav
					// Nastavení slova .HI. na displeji
					dispData[0] = getChar('.');
