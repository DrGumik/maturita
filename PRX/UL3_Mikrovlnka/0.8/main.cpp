...
int toggleMotor(int shiftMotor, int isMotorOff, int isBulbOff, int& motorToggleCounterOff, int& motorToggleCounterOn) {
	...
	if (isMotorOff) {
		...
		//if (isBulbOff == 0) tmp = pwmMasks[3]; // Pokud je zarovka zapla, tak nacte masku ZAROVKA ON -> motor touto maskou je vyplej
		//else tmp = pwmMasks[1]; // Pokud je zarovka vypla, tak nacte masku MOTOR OFF I ZAROVKA OFF
		...
		//if (isBulbOff == 0) tmp = pwmMasks[2]; // Pokud je zarovka zapla, tak nacte masku ZAROVKA ON i MOTOR ON
		//else tmp = pwmMasks[0]; // Pokud je zarovka vypla, tak nacte masku MOTOR ON
...
int toggleBulb(int shiftBulb, int isBulbOff, int isMotorOff, int& bulbToggleCounterOff, int& bulbToggleCounterOn) {
	...
	if (isBulbOff) {
		...
		//if (isMotorOff == 0) tmp = pwmMasks[0]; // Pokud je motor zaplej, tak nacte masku MOTOR ON -> zarovka touto maskou je vypla
		//else tmp = pwmMasks[1]; // Pokud je motor vyplej, tak nacte masku MOTOR OFF i ZAROVKA OFF
		...
		//if (isMotorOff == 0) tmp = pwmMasks[2];  // Pokud je motor zaplej tak nacte masku ZAROVKA ON i MOTOR ON
		//else tmp = pwmMasks[3]; // Pokud je motor vyplej tak nacte masku ZAROVKA ON
		...
		switch (pressedKey) {
			case 'w':
				if (shiftMotor <= 50 && shiftMotor >= 10) {
					...
					printf("\n\rAktualni rychlost:\x1B[33m %i%\033[0m", 110-shiftMotor*2);
				} else printf("\n\r\x1B[31m!! Nelze zvysit rychlost otaceni !!\033[0m");
				break;
			case 's':
				if (shiftMotor <= 45 && shiftMotor >= 5) {
					...
					printf("\n\rAktualni rychlost:\x1B[33m %i%\033[0m", 110-shiftMotor*2);
				} else printf("\n\r\x1B[31m!! Nelze snizit rychlost otaceni !!\033[0m");
				break;
			case 'q':
				break;
			default:
				printf("\n\r\x1B[31m!! Stiskl jsi nepovolenou klavesu !!\033[0m");
				break;
		}
	}
...
int main()
{
	...
	printf("\n\r\x1B[33m[--------------------------------------------------]\033[0m");
	printf("\n\r Zakladni ovladani: ");
	printf("\n\r   Stisk klavesy \x1B[33mW\033[0m nebo \x1B[33mw\033[0m  ->  zvysi rychlost otaceni talire o 10% ");
	printf("\n\r   Stisk klavesy \x1B[33mS\033[0m nebo \x1B[33ms\033[0m  ->  snizi rychlost otaceni talire o 10% ");
	printf("\n\r   Stisk klavesy \x1B[33mQ\033[0m nebo \x1B[33ms\033[0m  ->  ukonci program ");
	printf("\n\r\x1B[33m[--------------------------------------------------]\033[0m");
